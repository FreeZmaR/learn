...
function get_pop_com()
{
    ...
    mysqli_set_charset($link, "utf8"); 
    $iter = 0;
   // $query = "SELECT * FROM items ORDER BY buy DESC"; - выборка по покупкам
    $query = "SELECT * FROM items ORDER BY buy DESC"; // - выборка по просмотрам
    if($result = mysqli_query($link, $query)){
        while($row = mysqli_fetch_assoc($result)){
            foreach($row as $key => $val)
            {
...
//возвращает куски контента строкой
function get_content($file, $content){
    ...
    if(count($content) != null){
        foreach($content as $key => $val){
            if($key != 'basket'){
                ...
                $key = "((".strtoupper('count')."))";
                $template = str_replace($key, $content['basket']['count'], $template);
            }
        }
    }
    return $template;
...
function view($id, $view,$table)
{
    $view+=1;
    $query = "UPDATE ".$table." SET view='".$view."' WHERE id=".$id;
    $link = mysqli_connect(host, db_user, db_pw, db);
        if($result = mysqli_query($link, $query)){
